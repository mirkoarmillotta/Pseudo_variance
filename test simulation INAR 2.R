...
## regarding simulation with INAR(2).
##
...
# Generate data from INAR(2) with Poisson thinning and errors
rpinar<-function(n,a1, a2, lamda)
{
  x<-matrix(0,n)
  epsilon<-rpois(n,lamda)
  x_1<-rpois(1,lamda/(1-a1-a2))
  x0<-rpois(1,lamda/(1-a1-a2))
  x[1]=thinning_pois(a1,x0) + thinning_pois(a2,x_1) + epsilon[1]
  x[2]=thinning_pois(a1,x[1]) + thinning_pois(a2,x0) + epsilon[2]
  
  for (i in 3:n){
    x[i]=thinning_pois(a1,x[i-1]) + thinning_pois(a2,x[i-2]) + epsilon[i]
  }
  
  return(x)
}

# Generate data from INAR(2) with NB thinning and Poisson errors
rp_inar<-function(n,a1,a2, lamda, rr)
{
  x<-matrix(0,n)
  epsilon<-rpois(n,lamda)
  x_1<-rpois(1,lamda/(1-a1-a2))
  x0<-rpois(1,lamda/(1-a1-a2))
  x[1]=thinning_nb(a1, x0, rr)+thinning_nb(a2, x_1, rr)+epsilon[1]
  x[2]=thinning_nb(a1, x[1], rr)+thinning_nb(a2, x0, rr)+epsilon[2]
  
  for (i in 3:n){
    x[i]=thinning_nb(a1, x[i-1], rr)+thinning_nb(a2, x[i-2], rr)+epsilon[i]
  }
...
QML2 = function(y,theta){
  a1=exp(theta[1])/(1+exp(theta[1]))
  a2=exp(theta[2])/(1+exp(theta[2]))
  la=exp(theta[3])
  v=exp(theta[4])
  b1=exp(theta[5])
  b2=exp(theta[6])
...
sc_QML2 = function(theta, y){
  ...
  mt=a1*y1+a2*y2+la
  vt=b1*y1+b2*y2+v
  dmt = cbind(y1,y2,1,0,0,0)
  dvt = cbind(0,0,0,1,y1,y2)
  for(j in 1:6) ss[j] = sum( -1/(2*vt)*dvt[,j]+(y0-mt)/vt*dmt[,j]+(y0-mt)^2/(2*vt^2)*dvt[,j] )
  -ss
}
...
freq <- function(x, alpha){
  z <- vector()
  s <- sum(!is.na(x))
  z[1]<-sum(x>qchisq(alpha[1],2), na.rm = T)/s
  z[2]<-sum(x>qchisq(alpha[2],2), na.rm = T)/s
  z[3]<-sum(x>qchisq(alpha[3],2), na.rm = T)/s
  return(z)
}
...
ri <- 0.5
score[[j]] <- matrix(NA, B, 6)
QML2_est[[j]] <- matrix(NA, B, 6)
I[[j]] <- list()
QML2_se[[j]] <- matrix(NA, B, 6)
QML2_t[[j]] <- matrix(NA, B, 6)
...
for(i in 1:B){
  ...
  theta=c(a1,a2,la)
  ini2=c(log(theta[1]/(1-theta[1])),log(theta[2]/(1-theta[2])),
         log(theta[3]), log(la),log(a1+a1^2/ri),log(a2+a2^2/ri))
  
  est2 <- optim(par = ini2, fn = QML2, method = "BFGS", hessian=F, y=y,
                control = list(ndeps=rep(1e-5,6), reltol=1e-12, maxit=1000))
  
  score[[j]][i,] = sc_QML2(y=y, theta=est2$par)
  
  alpha1 = exp(est2$par[1])/(1+exp(est2$par[1]))
  alpha2 = exp(est2$par[2])/(1+exp(est2$par[2]))
  omega1 = exp(est2$par[3])
  omega2 = exp(est2$par[4])
  beta1 = exp(est2$par[5])
  beta2 = exp(est2$par[6])
  
  QML2_est[[j]][i,] = c(alpha1, alpha2, omega1, omega2, beta1, beta2)
  ...
  g = c(beta1 - alpha1, beta2 - alpha2)
  G = c(-1,0,0,0,1,0)
  G = rbind(G,c(0,-1,0,0,0,1))
  try(S <- solve(G%*%I[[j]][[i]]%*%t(G)))
  W[i,j] = t(g) %*% S %*% g
...
library(xtable)
print(xtable(estt, type = "latex", digits=c(4,4,4,4,4,4,4)),
      include.rownames=FALSE, file = "Tab_est2.tex")
...
print(xtable(chit, type = "latex", digits=c(4,4,4,4,4,4)),
      include.rownames=FALSE, file = "Tab_size2.tex")
...
for(j in 1:length(r)){
  ...
  for(i in 1:B){
    ...
    y=rp_inar(n,a1,a2,la,ri) # for NB thinning
...
ov = 1-a1/(a1+a1^2/r) # For NB thinning
ov = c(ov, 0)
...
legend(0.3, 0.25, legend=c("T=100", "T=250", "T=500", "T=1000", "T=2000"),
